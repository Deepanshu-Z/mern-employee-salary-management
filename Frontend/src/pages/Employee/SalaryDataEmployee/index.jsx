...
const SalaryDataEmployee = () => {
  const [dataSalaryEmployee, setSalaryDataEmployee] = useState([]);
  ...
  const onSubmitPrint = async () => {
    ...
    try {
      ...
      if (yearData.length > 0 && monthData.length > 0 && nameData.length > 0) {
        navigate(`/data-salary-employee/print-page?month=${dataMonth}&year=${dataYear}&name=${employee_name}`);
      } else {
